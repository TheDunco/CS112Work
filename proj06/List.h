 ...
 *  	remove() (and test)
 *
 *  Updated for CongaLine on 17 March 2019 by Duncan Van Keulen (djv78)
 */
...
public:
	...
	// CongaLine methods
	bool insertAfter(const string& otherPersonsName, const string& yourName);
	bool insertBefore(const string& otherPersonsName, const string& yourName);
...
/*
 * Inserts a person after another in a CongaLine
 * @param const string& otherPersonsName: The name of the person to insert after
 * @param const string& yourName: The name of the person to be inserted
 * Precondition: Conga line must exist and otherPersonsName must be in the CongaLine
 * Postcondition: yourName will be inserted directly after otherPersonsName in the CongaLine
 * Returns: Boolean of whether or not the person was in the list, and therefore if anything was added
 * @author: Duncan Van Keulen
 */
template<class Item>
bool List<Item>::insertAfter(const string& otherPersonsName, const string& yourName) {
	unsigned index;
	try {
		index = getIndexOf(otherPersonsName);	  // get the index of the other person (if existent)
	} catch (invalid_argument&) {
		return false;
	}
	insert(yourName, (index + 1));			  		  // use list insert to insert item at the next index
	return true;
}

/*
 * Inserts a person after before in a CongaLine
 * @param const string& otherPersonsName: The name of the person to insert before
 * @param const string& yourName: The name of the person to be inserted
 * Precondition: Conga line must exist and otherPersonsName must be in the CongaLine
 * Postcondition: yourName will be inserted directly before otherPersonsName in the CongaLine
 * Returns: Boolean of whether or not the person was in the list, and therefore if anything was added
 * @author: Duncan Van Keulen
 */
template<class Item>
bool List<Item>::insertBefore(const string& otherPersonsName, const string& yourName) {
	unsigned index;
	try {
		index = getIndexOf(otherPersonsName);	  // get the index of the other person (if existent)
	} catch(invalid_argument&) {
	...
	insert(yourName, index);			  		  // use list insert to insert item at the previous index
	return true;
}

/*
 * Insertion operator overloading
 * Writes the List to a stream in CongaLine format
 * @param ostream& out: The ostream for the CongaLine/List to be written to
 * @param List<Item>& rhs: the List on the right hand side of the operator to be written
 * Precondition: List needs to exist, stream needs to be valid
 * Postcondition: List items will be written to stream separated by "=" (for CongaLine)
 * Returns ostream&: The ostream object that was passed to the function: to enable chaining of "<<"
 * @author: Duncan Van Keulen
 */
template<class Item>
ostream& operator<<(ostream& out, List<Item>& rhs) {
	rhs.writeTo(out, "=");
	return out;
}
