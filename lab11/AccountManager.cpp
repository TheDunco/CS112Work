...
void AccountManager::changeUserName(const string& oldUserName, const string& newUserName) {
	...
	if ( it == myAccounts.end() ) {
		throw Exception("AccountManager::changeUserName", "Account not present");
	} else {
		// retain the password
		string pass = it->second;
		// erase the old entry
		myAccounts.erase(it);
		// create a new account with the new username and old password
		createNewAccount(newUserName, pass);
	}
}
...
void AccountManager::deleteAccount(const string& userName) {
	...
	if ( it ==  myAccounts.end() ) {
		throw Exception("AccountManager::deleteAccount", "Account not present");
	} else {
...
void AccountManager::changePassword(const string& userName,
	...
	map<string, string>::iterator it = myAccounts.find(userName);
	if ( it ==  myAccounts.end() ) {
		throw Exception("AccountManager::changePassword", "Account not present");
	} else {
