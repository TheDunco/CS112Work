...
/* Song input method...
 * @param: in, an istream
 * Precondition: the file contains the title, artist, and year data for a Song.
...
/* Overload the equality operator to work with Song objects
 * @param: Song, a Song object
 * @param: song2, a different Song object
 * Return: boolean of song operator, true if songs are exactly the same
 */
bool Song::operator==(const Song& song2) const {
	if ( this->getTitle() != song2.getTitle() ) { return false; }
	if ( this->getArtist() != song2.getArtist() ) { return false; }
	if ( this->getYear() != song2.getYear() ) { return false; }
	return true;
}
